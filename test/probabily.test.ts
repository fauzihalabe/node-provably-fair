...
describe('ProvablyFair', () => {
    ...
    test('Deve gerar 10 números aleatórios consecutivos baseados no hash do bloco Bitcoin e verificar cada giro', async () => {
        ...
        for (let i = 1; i <= 10; i++) {
            const { randomNumber, serverSeed, secretSalt, nonce, publicHash, bitcoinHash } = await provablyFair.generateProvably(clientSeed);
            ...
            // console.log(`Roll ${i} - BTC Hash:`, bitcoinHash);
